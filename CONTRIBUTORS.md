...
- [Issue #1](https://github.com/epoko77-ai/im-not-ai/issues/1) "실전 사용 후기 + 개선 제안 4건 — 단행본 원고 8.5만 자 적용 결과"
  - v1.2 권한 위계 §1~§6 신설 동기 (`.agents/skills/humanize-korean/references/ai-tell-taxonomy.md`)
  - 작가 voice profile 실험과 외부 어댑터 정책 제안
...
- [PR #3](https://github.com/epoko77-ai/im-not-ai/pull/3) "v1.2 권한 위계 다운스트림 어댑터 reference"
  ...
  - Telemetry 정책 (voice profile log)
  - Hard-block은 caller/adapter 책임 명시
  - 어댑터 reference 본체는 v1.x 실험 기록으로 보존

**관련 commit**: `bfcf676`, `9f39ce0`, `81fd1b9`

### [@gaebalai](https://github.com/gaebalai) (AI-fluent liberal arts Engineer)

**기여**: LICENSE 누락 지적 + v1.x command/plugin/자동 설치기 reference + 외부 distribution channel 운영.
...
- [Issue #6](https://github.com/epoko77-ai/im-not-ai/issues/6) "슬래시커맨드가 있으면 더 좋을것 같아요"
  - v1.x 사용자 command 흐름 제안 (`9054518`)
...
- [`gaebalai/im-not-ai`](https://github.com/gaebalai/im-not-ai) 포크
  - v1.x plugin/marketplace 패키징 reference
...
- **새 AI 티 패턴 제보** — `.agents/skills/humanize-korean/references/ai-tell-taxonomy.md` 후보로 Issue 등록 (실증 사례 2건+ 첨부 시 승격 검토)
- **외부 회귀 케이스 제공** — 다른 작가/장르의 입력 텍스트로 Fast/Strict 비교 리포트 제공
- **사용성 개선 제안** — Codex skill, plugin, prompt template, 자동화 reference 등
